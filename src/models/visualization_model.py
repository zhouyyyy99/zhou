...
def padding_mask(lengths, max_len=None):
    ...
    if max_len==None:
        ...
        max_len = max_len
    return torch.zeros(max_len) == torch.zeros(max_len)
    ...
    if (task == "classification") or (task == "regression"):
        ...
        if config['model'] == 'LINEAR':
            ...
            return TSTransformerEncoderClassiregressor(feat_dim, max_seq_len, config['d_model'],
                                                       ...
                                                       activation=config['activation'],
                                                       norm=config['normalization_layer'], freeze=config['freeze'],
                                                       num_linear_layer=config['num_linear_layer'])
    else:
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
                 num_linear_layer, num_subset=3, diffusion=2,
        ...
        if num_linear_layer == 1:
            self.output_layer = self.build_output_module(d_model, self.max_len, num_classes, num_layer=num_linear_layer)
        if num_linear_layer == 2:
            self.output_layer1, self.output_layer2 = self.build_output_module(d_model, self.max_len, num_classes,
                                                                              num_layer=num_linear_layer)

        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


    def build_output_module(self, d_model, max_len, num_classes, num_layer=1):
        if num_layer == 1:
        ...
        if num_layer == 2:
            output_layer1 = nn.Linear(d_model * max_len, 128)
            output_layer2 = nn.Linear(128, num_classes)
    ...
    def forward(self, X):
        ...
        # permute because pytorch convention for transformers is [seq_length, batch_size, feat_dim]. padding_masks [batch_size, feat_dim]
        # print('----------------in TSTransformerEncoderClassiregressor -----------------')
        # padding_masks = X[1]
        # X = X[0]
        # print('input x.shape is:')
        # print(X.shape)

        # all_true_list = [True for _ in range(X.shape[0]*128)]
        # all_true_array = np.array((all_true_list)).reshape((X.shape[0], 128))
        # all_true_array[:, -12:] = False
        # padding_masks = torch.from_numpy(all_true_array).to(self.device)

        padding_masks = padding_mask(torch.tensor(116, dtype=torch.int16), max_len=128)
        # X = X.unsqueeze(0)  # 没有用？
        padding_masks = padding_masks.unsqueeze(0)
        ...
        # X = X.expand(32, 116, 116)
        # padding_masks = padding_masks.expand(32, 116)
        ...
        inp = self.pos_enc(inp)  # add positional encoding  # if adding positional encoding
        # NOTE: logic for padding masks is reversed to comply with definition in MultiHeadAttention, TransformerEncoderLayer
        # padding_masks = self.padding_masks
        # print('padding masks is:')
        # print(padding_masks)
        output = self.transformer_encoder(inp, src_key_padding_mask=~padding_masks)  # (seq_length, batch_size, d_model)
        # print('output shape')
        # print(output.shape)
