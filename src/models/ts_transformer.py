...
class Diffusion(nn.Module):
    def __init__(self, d_model, num_subset=3, diffusion=2, attentiondrop=0):
        ...
        self.out_nets = nn.Sequential(
            nn.Conv2d(num_subset, 1, kernel_size=2, stride=1, padding=1, padding_mode='zeros', bias=True),
            nn.BatchNorm2d(1),
        )

    def forword(self, inp):
        dism = torch.cdist(inp, inp, p=2)  # L2正则化计算矩阵相似度
        # dism.shape : [batch_size, seq_length, seq_length]
        # dism.shape : [batch_size, d_model, d_model]
        print('dism.shape')
        print(dism.shape)

        dism = torch.pow(dism / inp.shape[2], 2)  # 计算指数 首先将 dism 中的每个数除以c*t，再平方
        # shape : 不变
        ...
        attention = self.out_nets(attention)
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def forward(self, X, padding_masks):
        ...
        inp = inp.permute(1, 2, 0)  # [batch_size, d_model, seq_length]
