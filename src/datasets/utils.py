...
def apply_slide_window_in_flatten_matrix(filename, silde_window_size, slide_window_stride, sequence_length, total_subjects):
    print('apply slide window in datasets')
    datas = np.load(filename)
    datas_flattens = datas[datas.files[0]]
    all_labels = datas[datas.files[1]]

    num_window = int((sequence_length - silde_window_size) / slide_window_stride) + 1

    patches = []
    all_df = np.zeros((total_subjects, num_window, silde_window_size), dtype=float)

    for i in range(datas_flattens.shape[0]):
        print(i)
        corr_flatten = datas_flattens[i] # 这个很长
        for j in range(num_window):
            patch = corr_flatten[j*slide_window_stride:j*slide_window_stride + silde_window_size]
            patches.append(patch)
        all_df[i] = np.array(patches, dtype=object)
        patches.clear()
    return all_df, all_labels


def get_flatten2matrix(up_flatten_data, size):
    """
    从上三角向量向量变回矩阵
    :param up_flatten_data(samples, data_dim), size
    :return: matrix (samples, size, size)
    """
    matrix = np.ones((up_flatten_data.shape[0], size, size))

    for j in range(up_flatten_data.shape[0]):
        matrix[j][np.triu_indices(size, 1)] = up_flatten_data[j]

    for i in range(up_flatten_data.shape[0]):
        matrix[i][np.tril_indices(size, -1)] = matrix[i].T[np.tril_indices(size, -1)]

    return matrix


def read_indices(json_file_path):
    f = open(json_file_path, 'r')
    content = f.read()
    indices_dict = json.loads(content)
