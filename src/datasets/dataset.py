...
class ImputationDataset(Dataset):
    ...
    def __init__(self, data, indices, feature_mask_index, mean_mask_length=3, masking_ratio=0.15,
...
class ClassiregressionDataset(Dataset):
    ...
    def __init__(self, data, indices, feature_indices=None):
        super(ClassiregressionDataset, self).__init__()

        self.data = data  # this is a subclass of the BaseData class in data.py
        self.IDs = indices  # list of data IDs, but also mapping between integer index and ID
        self.feature_indices = feature_indices
        # print('np.sum(self.feature_indices)')
        # print(np.sum(self.feature_indices))
        
        if self.feature_indices is not None:            
            self.feature_array = self.data.feature_df.loc[self.IDs].values  # datafarme转成array才可以应用mask
            feature_array_temp = []
            temp = []
            for item in self.feature_array:
                for i in item:
                    temp.append(i)
                feature_array_temp.append(np.array(temp))
                temp = []
            self.feature_array = np.array(feature_array_temp)  # 真正转成功了array

            self.feature_array = self.feature_array.reshape(-1, 116, 116) 
            self.mask = []
            for i in range(self.feature_array.shape[0]):  # 做整个数据的mask
                self.mask.append(feature_indices)
            self.mask = np.array(self.mask)
            # self.feature_array = ma.array(self.feature_array, mask=self.mask)  
            self.feature_array = self.feature_array * self.mask  # 应用 mask 直接相乘，变成 0

            # mask 之后要转成dataframe
            self.feature_df = from_array_to_dataframe(self.feature_array)  # 转成 dataframe
            # print('----- feature_df in dataset.py ------------')
            # print(self.feature_df)
            # print(type(self.feature_df))
            # print('datasets apply mask feature indices')
    ...
    def __getitem__(self, ind):
        ...
        if self.feature_indices is not None:
            ...
            for item in X:
                for i in item:
                    temp.append(i)
                X_array_temp.append(np.array(temp))
                temp = []
            X = np.array(X_array_temp)  # 真正转成功了array
        else:
            X = self.feature_df.loc[self.IDs[ind]].values  # (seq_length, feat_dim) array
