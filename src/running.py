...
from sagemaker_platform_code.src.utils import utils, analysis
from sagemaker_platform_code.src.models.loss import l2_reg_loss
from sagemaker_platform_code.src.datasets.dataset import ImputationDataset, TransductionDataset, ClassiregressionDataset, collate_unsuperv, collate_superv
...
def setup(args, k=0):
    ...
    output_dir = os.path.join(output_dir, config['experiment_name']+'_fold{}'.format(k))
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        if self.classification:
            ...
            if self.model.num_classes == 2:
                ...
                y_pred = np.around(probs[:, 1], 0).astype(int)
                # print(targets)
                # print()
                # print(y_pred)
                sensitivity = sklearn.metrics.precision_score(targets, y_pred)
                specificity = sklearn.metrics.recall_score(targets, y_pred)
                self.epoch_metrics['SEN'] = sensitivity
                self.epoch_metrics['SPE'] = specificity
