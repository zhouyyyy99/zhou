...
if __name__ == '__main__':
    ...
    print('---------------------------------------------------------')
    k_fold_acc_list.sort(reverse=True)
    k_fold_sen_list.sort(reverse=True)
    k_fold_spe_list.sort(reverse=True)
    print('{} fold result is :'.format(k))
    if(k>=3):
        print('acc:{}'.format(np.mean(k_fold_acc_list[:3])))
        print('sen:{}'.format(np.mean(k_fold_sen_list[:3])))
        print('spe:{}'.format(np.mean(k_fold_spe_list[:3])))
        print('auroc:{}'.format(np.mean(k_fold_auroc_list[:3])))
        print('sauprc:{}'.format(np.mean(k_fold_auprc_list[:3])))
    else:
