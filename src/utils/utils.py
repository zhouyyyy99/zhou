...
def t_test_selected(p: float, data_filename, train_indices, dimension=2):
    """
    根据指定的p值获取FC
    """
    data = np.load(data_filename)

    train_x = pd.DataFrame(data[data.files[0]]).loc[train_indices].values
    train_y = pd.DataFrame(data[data.files[1]]).loc[train_indices].values[:, 0]
    index = []
    ...
    if dimension == 1:
        return index
    if dimension == 2:
        data_temp = np.zeros(6670)
        data_temp[np.transpose(index)] = 1
        mask = np.zeros((116, 116))
        mask[np.triu_indices(116, 1)] = data_temp
        mask[np.tril_indices(116, -1)] = mask.T[np.tril_indices(116, -1)]
        return mask

    return None

def plot_loss(loss_list, save_path, label_name='train loss'):
